import os
import ctypes
from ctypes import (c_int, c_uint, c_bool, c_float, c_char_p,
                    POINTER, byref)
...
# Define the data types used by the DLL.
DEVID = c_uint
LVSTATUS = c_uint
STRING = c_char_p
...
fnLMS_GetNumDevices = getDllObject('fnLMS_GetNumDevices',
        [], restype=c_int)
fnLMS_GetDevInfo = getDllObject('fnLMS_GetDevInfo',
        [POINTER(ACTIVEDEVICES)], restype=c_int)
fnLMS_GetModelName = getDllObject('fnLMS_GetModelName',
        [DEVID, STRING], restype=c_int)
fnLMS_GetSerialNumber = getDllObject('fnLMS_GetSerialNumber',
        restype=c_int)
...
# VNX_FSYNSTH_API LVSTATUS fnLMS_SetFastPulsedOutput(DEVID deviceID, float
# pulseontime, float pulsereptime, bool on);
...
fnLMS_SetUseInternalRef = getDllObject('fnLMS_SetUseInternalRef',
        [DEVID, c_bool])
fnLMS_SetUseExternalPulseMod = getDllObject('fnLMS_SetUseExternalPulseMod',
        [DEVID, c_bool])
fnLMS_SetFastPulsedOutput = getDllObject('fnLMS_SetFastPulsedOutput',
        [DEVID, c_float, c_float, c_bool])
...
fnLMS_GetUseInternalRef = getDllObject('fnLMS_GetUseInternalRef',
        restype=c_int)
...
fnLMS_GetPulseOnTime = getDllObject('fnLMS_GetPulseOnTime',
        restype=c_float)
fnLMS_GetPulseOffTime = getDllObject('fnLMS_GetPulseOffTime',
        restype=c_float)
fnLMS_GetPulseMode = getDllObject('fnLMS_GetPulseMode',
        restype=c_int)
fnLMS_GetUseInternalPulseMod = getDllObject('fnLMS_GetUseInternalPulseMod',
        restype=c_int)
...
class LabBrick_Synthesizer():
    ...
    def __init__(self, bTestMode=False):
        """
        The init case defines a session ID, used to identify
        the instrument.
        """
    ...
    def getListOfDevices(self):
        ...
        for n1 in range(nDev):
            nameBuffer = ctypes.create_string_buffer(MAX_MODELNAME)
    ...
    def setInternalPulseMod(self, dOntime, dReptime, bOn=False):
        status = fnLMS_SetFastPulsedOutput(self.device_id,
                                           c_float(dOntime),
                                           c_float(dReptime),
                                           c_bool(bOn))
        self.check_error(status)
